...
//房间状态
if ($cfg['config']['state'] == '2' and $_SESSION['room_' . $cfg['config']['roomid']] != true) {
    header("location:../login.php?rid=" . $rid);
    exit();
}
if ($cfg['config']['state'] == '0') {
    exit("<script>location.href='../error.php?rid=" . $rid . "&msg=系统处于关闭状态！请稍候……'</script>");
    exit();
}
if ($cfg['config']['state'] == '3') {
    system_open_time($cfg['config']['sysstart'], $cfg['config']['sysend']);
}
//游客登录
...
<head>
    ...
    <script>
        ...
        var RoomInfo = {
            ...
            loginguest: '<?=$cfg['config']['loginguest']?>',
    ...
    <div id="redpackage" style="display: none">
        ...
        <div class="self-weui-cell">
            ...
            <div class="weui-cell__bd">
                <input maxlength="2" class="weui-input" placeholder="填写个数" id="redpackage-num"
                       onfocus="hideErrorTip('num')">个
            </div>
        </div>
        ...
        <div class="self-weui-cell">
            ...
            <div class="weui-cell__bd">
                <input maxlength="4" class="weui-input" placeholder="填写金额" id="redpackage-amount"
                       onfocus="hideErrorTip('amount')">元
            </div>
        </div>
    ...
    <article>
                ...
                <div class="login-info">
                    ...
                    <div class="account-info-box">
                        <ul class="account-list">
                            <!--            <li class="account-item account-amount">-->
                            <!--              <a href="javascript:void(0)">余额:<span class="account-money">-->
                            <? //=$userinfo['moneyred']?><!--</span> <span class="withdraw" onclick="withdraw(--><? //=$_SESSION['login_uid']?>
                            //)">提现</span></a>
                            //            </li>
                            <li class="account-item">
                                <a href="javascript:void(0)">积分:<span
                                            class="account-credit"><?= $userinfo['redbags'] ?></span></a>
                            </li>
                            <li class="account-item oneself-profile">
                                <a href="editprofile.php?rid=<?= $rid ?>">个人资料</a>
                            </li>
                            <li class="account-item exit-btn">
                                <a href='../minilogin.php?act=logout&rid=<?= $rid ?>'>退出登录</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <a id="video-show" class="videoShow" href="javascript:void(0);" title="显示视频"></a>
                <a id="video-hide" class="videoShow" href="javascript:void(0);" title="隐藏视频" style="display:none"></a>
            </section>
            <section class="video-container">
                <div id="logo"><img src="<?= $cfg['config']['logo'] ?>" alt="www.46465.com" height="260" width="150">
                </div>
                <div class="videoTitle">
                    <!-- <img src="./images/group.png"> --><span>在线</span>
                    <span id="OnlineUserNum"></span><span>人</span>
                </div>
                <!-- 视频 -->
                <div id="mvMask"></div>
                <div class="video-box">
        ...
        <section class="tab-content">
            <div class="top" id="announcement">
                <marquee scrollamount="3" id="msg_tip_show" behavior="scroll" loop="infinite"><?php
            ...
            <div class="abs-top-left" id="whocomein"
                 style="bottom: 186px;display:block;position: absolute;left:0;z-index:9999;">
            </div>
            <!-- 在线管理 -->
            ...
            <div class="signin-container">
                <img src="./images/signin-icon.png">
            </div>
            <? if ($global['config']['goldegg'] == '1') { ?>
                <div class="pound-gold-egg">
                    <img src="./images/egg.png">
                </div>
            <? } ?>
            <div class="leftAside">
                <img src="./images/hall_redpackage_icon.png">
            </div>
        </section>
    </article>
    ...
    <div class="more-function-container" style="display:none" id="moreFunction">
        <dl>
            ...
            <? if (check_auth('send_img')) { ?>
                <dd>
                    <div class="img-container" id="UploadImgBtn" onclick="$('#filedata').click()"><img
                                src="./images/upload-img.png"></div>
                    <div class="text-container"><span>图片</span></div>
                </dd><? } ?>
            <? if ($room_admin) { ?>
                <dd>
                <div class="img-container" id="shutBtn"><img src="./images/shut.png"></div>
                <div class="text-container"><span id="shutinfo"><? if ($cfg['config']['banspeak'] == 1) {
                            echo '取消禁言';
                        } else {
                            echo '禁言';
                        } ?></span></div></dd><? } ?>
        ...
        <input id="filedata" type="file" size="20" name="filedata" class="input" style="display:none"
               onchange="uploadAvatar('filedata', '#messageEditor')">
    </div>
...
<script>
    var barrage = false;
    var commonExpression, ISSHOWEGG = <?=$global['config']['goldegg']?> ==
    '1' ? true : false;
