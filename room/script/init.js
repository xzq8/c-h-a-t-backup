...
if(RoomInfo.autoplan==1){
  ...
  //  }, 60000);
    ...
    MsgAutoScroll();
    $('#RoomMV .btn-right a').click(function() {
    	var id = $(this).data('id');
    	var arr = ['idLotteryLive', 'idTextLottery', 'idBet', 'OnLine_MV'];
    	$(this).addClass('active').siblings().removeClass('active');
    	$('.NoticeList').show();
    	$.each(arr, function(idx, item){
    		if (id == item) {
    			$('#' + item).show();
    			if(id == 'idTextLottery' || id == 'idBet') {
    				$('.NoticeList').hide();
    			}
    		} else {
    			$('#' + item).hide();
...
function OnResize(){
    ...
    $('.ck-slide').height($('.NoticeList').width()*0.35);
    $('#OnLine_MV').css("height",cH-$('#UI_Head').height()- 35 - $('.NoticeList').height());

    $('#idLotteryLive').css("height",cH-$('#UI_Head').height()- 35 - $('.NoticeList').height());

    $('#idTextLottery').css("height",cH-$('#UI_Head').height()- 35);
    $('#idTextLottery iframe').css("height",cH-$('#UI_Head').height()- 35);

    $('#idBet').css("height",cH-$('#UI_Head').height()- 35);
    $('#idBet iframe').css("height",cH-$('#UI_Head').height()- 35);
