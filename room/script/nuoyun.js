  ...
  $(".hongBao").click(function() {
     var a = '<div id="hongBaoClick">';
        ...
        $(".sethongBao").click(function() {
            ...
            a += '<div class="mt10"><div class="registerNewconter" style="padding-right:60px;">个数：<input type="text" name="realnumber" id="realnumber" maxlength="2" value="请填写红包个数" title="请填写红包个数"> 个</div><p class="envelopetip" id="realnumbertip" style="margin-left:97px;"></p></div>',
...
$(document).ready(function() {
    $('.js-main-meta-toggle').click(function() {
        ...
        if( _mLeft < 0) {
            ...
            $('#UI_Central').width($('#UI_Central').width() - 242);
...
function handleData (type, data) {
    ...
    if (type == 'contribution') {
      $.each(data, function(idx, item) {
        ...
        _html += '<li><span class="item-num ' + num + '">' + (idx + 1) + '</span>' +
                  '<span class="avatar avatar-one"><a href="javascript:void(0)" target="_blank"><img src="./face/img.php?t=p1&u=' + item.uid + '" alt="" /></a></span> ' +
                  '<span class="level-icon"><i class="level"><img src="room/images/ranklist/level_1.png" /></i></span> ' +
                  '<span class="week-rank-name"><a href="javascript:void(0)" target="_blank">' + item.nickname + '</a></span><span class="fans-wrap">' + item.num +'</span></li>';
		});
      ...
      $.each(data, function(idx, item) {
        ...
        _html += '<li><span class="item-num ' + num + '">' + (idx + 1) + '</span>' +
                  '<span class="avatar avatar-one"><a href="javascript:void(0)" target="_blank"><img src="./face/img.php?t=p1&u=' + item.uid + '" alt="" /></a></span> ' +
                  '<span class="week-rank-name"><a href="javascript:void(0)" target="_blank">' + item.nickname + '</a></span><span class="third-col">' + item.onlinetime +'</span></li>';
      });
    } else {
      $.each(data, function(idx, item) {
        var num = '';
        ...
        _html += '<li><span class="item-num ' + num + '">' + (idx + 1) + '</span>' +
                  '<span class="avatar avatar-one"><a href="javascript:void(0)" target="_blank"><img src="./face/img.php?t=p1&u=' + item.uid + '" alt="" /></a></span> ' +
                  '<span class="week-rank-name"><a href="javascript:void(0)" target="_blank">' + item.nickname + '</a></span><span class="third-col">' + item.num +'人</span></li>';
      }); 
    }

    if (_html == '') {
        _html += '<li class="no-data"><span>暂无数据</span></li>'
    }

    return _html;
}

$("#navTab>li").each(function(i){
  $(this).hover(function(){
    var url = '';
    var isLoad = true;
    $("h3").removeClass("achover");
    if (i == 0) {
      url = 'ajax.php?act=DevoteRank&roomid=' + My.rid + '&type=week';
      if ($('.sub #con-week')[0].children.length > 0) {
        isLoad = false;
      }
    } else if (i == 1) {
      url = 'ajax.php?act=DurationRank';
      if ($('.sub ul.rank-list-last')[0].children.length > 0) {
        isLoad = false;
      }
    } else if (i == 2) {
      url = 'ajax.php?act=ShareRank';
      if ($('.sub ul.rank-list-share')[0].children.length > 0) {
        isLoad = false;
      }
    }
    if (isLoad) {
      $.ajax({
          ...
          success:function(redata){
            if (i == 0) {
              var conhtml = handleData('contribution', redata);
              $(".sub #con-week").html(conhtml);
              initConHeader();
            } else if (i == 1) {
              var lasthtml = handleData('duration', redata);
              $(".sub .rank-list-last").html(lasthtml);
            } else {
              var sharehtml = handleData('share', redata);
              $(".sub .rank-list-share").html(sharehtml);
            }
          }
	})
    }
    $(this).children("h3").addClass("achover");
...
function initConHeader () {
    $(".contribution-head-container>li").each(function(i){
        $(this).click(function(){
            var url = '';
            var isLoad = true;
            var type = '';
            $(".contribution-head-item").removeClass("con-active");
            if (i == 0) {
                type="week";
                if ($('.sub ul#con-week')[0].children.length > 0) {
                    isLoad = false;
                } 
            } else if (i == 1) {
                type='mon';
                if ($('.sub ul#con-mon')[0].children.length > 0) {
                    isLoad = false;
                } 
            } else if (i == 2) {
                type='all';
                if ($('.sub ul#con-all')[0].children.length > 0) {
                    isLoad = false;
                } 
            }
            url = 'ajax.php?act=DevoteRank&roomid=' + My.rid + '&type=' + type;
            if (isLoad) {
              $.ajax({
                  url: url,
                  type:"get",
                  ...
                  success:function(redata){
                    if (i == 0) {
                      var weekhtml = handleData('contribution', redata);
                      $(".sub #con-week").html(weekhtml);
                    } else if (i == 1) {
                      var monhtml = handleData('contribution', redata);
                      $(".sub #con-mon").html(monhtml);
                    } else {
                      var allhtml = handleData('contribution', redata);
                      $(".sub #con-all").html(allhtml);
                    }
                  }
              })
            }
