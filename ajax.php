	...
	break;
        ...
        case "SignIn":
             ...
             $time=time(); 
                   ...
                   $db->query("update {$tablepre}sign set last_sign_time=$time,lx_sign=lx_sign+1 where userid='$uid'"); 
                            
                        }else{
                            //如果签到间隔大于一天
                          $persist=1;
                        $db->query("update {$tablepre}sign set last_sign_time=$time,lx_sign=1 where userid='$uid'");
                        }
                
             }else{ 
              //如果没有插入
              $persist=1;
              $db->query("insert into {$tablepre}sign(userid,last_sign_time,lx_sign,roomid)values('$uid','$time',1,'$rid')");
            }
         //插入签到信息
